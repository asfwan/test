...
var w2utils = (function () {
	...
	function isTime (val) {
		...
		val = $jquery.trim(val.replace('AM', ''));
		val = $jquery.trim(val.replace('PM', ''));
		// ---
		var tmp = val.split(':');
	...
	function stripTags (html) {
		...
		switch (typeof html) {
			case 'number':
				break;
			case 'string':
				html = $jquery.trim(String(html).replace(/(<([^>]+)>)/ig, ""));
				break;
			case 'object':
	...
	function transition (div_old, div_new, type, callBack) {
		var width  = $jquery(div_old).width();
		var height = $jquery(div_old).height();
		var time   = 0.5;
				
		if (!div_old || !div_new) {
			$jquery.error('Cannot do transition when one of the divs is null');
			return;
		}
		...
		setTimeout(function () {
			if (type == 'slide-down') {
				$jquery(div_old).css('z-index', '1019');
				$jquery(div_new).css('z-index', '1020');
			}
			if (div_new) {
				$jquery(div_new).css({ 
					'opacity': '1', 
			...
			if (div_old) {
				...
				if (div_old.parentNode) $jquery(div_old.parentNode).css({
					'-webkit-perspective': '',
					'-moz-perspective': '',
					'-ms-perspective': '',
					'-o-perspective': ''
		...
		function cross(property, value, none_webkit_value) {
			if (!$jquery.browser.webkit && typeof none_webkit_value != 'undefined') value = none_webkit_value;
	...
	function getSize (el, type) {
		var bwidth = {
			left: 	parseInt($jquery(el).css('border-left-width')) || 0,
			right:  parseInt($jquery(el).css('border-right-width')) || 0,
			top:  	parseInt($jquery(el).css('border-top-width')) || 0,
			bottom: parseInt($jquery(el).css('border-bottom-width')) || 0
		}
		var mwidth = {
			left: 	parseInt($jquery(el).css('margin-left')) || 0,
			right:  parseInt($jquery(el).css('margin-right')) || 0,
			top:  	parseInt($jquery(el).css('margin-top')) || 0,
			bottom: parseInt($jquery(el).css('margin-bottom')) || 0
		}
		var pwidth = {
			left: 	parseInt($jquery(el).css('padding-left')) || 0,
			right:  parseInt($jquery(el).css('padding-right')) || 0,
			top:  	parseInt($jquery(el).css('padding-top')) || 0,
			bottom: parseInt($jquery(el).css('padding-bottom')) || 0
		}
		switch (type) {
			...
			case 'width': 	return bwidth.left + mwidth.left + bwidth.right + mwidth.right + pwidth.right + pwidth.right + parseInt($jquery(el).width()); 
			case 'height': 	return bwidth.top + mwidth.top + bwidth.bottom + mwidth.bottom + pwidth.bottom + pwidth.bottom + parseInt($jquery(el).height());
...
$jquery.w2event = {
	...
	on: function (eventData, handler) {
		...
		if (typeof eventData.type == 'undefined') { $jquery.error('You must specify event type when calling .on() method of '+ this.name); return; }
		if (typeof handler == 'undefined') { $jquery.error('You must specify event handler function when calling .on() method of '+ this.name); return; }
		this.handlers.push({ event: eventData, handler: handler });
	},
	
	off: function (eventData, handler) {
		if (!$jquery.isPlainObject(eventData)) eventData = { type: eventData };
		eventData = $jquery.extend({}, { type: null, execute: 'before', target: null, onComleted: null }, eventData);
	
		if (typeof eventData.type == 'undefined') { $jquery.error('You must specify event type when calling .off() method of '+ this.name); return; }
	...
	trigger: function (eventData) {
		var eventData = $jquery.extend({ type: null, phase: 'before', target: null, stop: false }, eventData);
		...
		for (var h = this.handlers.length-1; h >= 0; h--) {
			...
			if ( (t.event.type == eventData.type || t.event.type == '*') 
				...
				if ($jquery.isPlainObject(ret)) { 
					$jquery.extend(eventData, ret);
...
(function () {
	...
	$jquery.fn.w2render = function (name) {
		if ($jquery(this).length > 0) {
			if (typeof name == 'string' && w2ui[name]) w2ui[name].render($jquery(this)[0]);
			if (typeof name == 'object') name.render($jquery(this)[0]);
		}
	}

	$jquery.fn.w2destroy = function (name) {
		if (typeof name == 'string' && w2ui[name]) w2ui[name].destroy();
		if (typeof name == 'object') name.destroy();
	}

	$jquery.fn.w2lite = function () {
	}
	
	$jquery.fn.w2tag = function (text, options) {
		if (!$jquery.isPlainObject(options)) options = {};
		if (!$jquery.isPlainObject(options.css)) options.css = {};
		if (typeof options['class'] == 'undefined') options['class'] = '';
		// remove all tags
		if ($jquery(this).length == 0) {
			$jquery('.global-tag').each(function (index, elem) {
				$jquery('#' + $jquery(elem).data('tagID')).removeClass( $jquery(elem).data('options')['class'] );
				clearInterval($jquery(elem).data('timer'));
				$jquery(elem).remove();
			});
			return;
		}
		return $jquery(this).each(function (index, el) {
			...
			if (text == '' || text == null || typeof text == 'undefined') {
				$jquery('#global-tag-'+tagID).css('opacity', 0);
				setTimeout(function () {
					// remmove element
					clearInterval($jquery('#global-tag-'+tagID).data('timer'));
					$jquery('#global-tag-'+tagID).remove();
				}, 300);
			} else {
				// remove elements
				clearInterval($jquery('#global-tag-'+tagID).data('timer'));
				$jquery('#global-tag-'+tagID).remove();
				// insert
				$jquery('body').append('<div id="global-tag-'+ tagID +'" class="global-tag" '+
				...
				var timer = setInterval(function () { 
					...
					if ($jquery(el).length == 0 || ($jquery(el).offset().left == 0 && $jquery(el).offset().top == 0)) {
						clearInterval($jquery('#global-tag-'+tagID).data('timer'));
						tmp_hide(); 
						return;
					}
					// monitor if moved
					if ($jquery('#global-tag-'+tagID).data('position') != ($jquery(el).offset().left + el.offsetWidth) + 'x' + $jquery(el).offset().top) {
						$jquery('#global-tag-'+tagID).css({
							'-webkit-transition': '.2s',
							'-moz-transition': '.2s',
							'-ms-transition': '.2s',
							'-o-transition': '.2s',
							left: ($jquery(el).offset().left + el.offsetWidth) + 'px',
							top: $jquery(el).offset().top + 'px'
						}).data('position', ($jquery(el).offset().left + el.offsetWidth) + 'x' + $jquery(el).offset().top);
					}
				}, 100);
				setTimeout(function () {
					if (!$jquery(el).offset()) return;
					$jquery('#global-tag-'+tagID).css({
						opacity: '1',
						left: ($jquery(el).offset().left + el.offsetWidth) + 'px',
						top: $jquery(el).offset().top + 'px'
					...
					.data('position', ($jquery(el).offset().left + el.offsetWidth) + 'x' + $jquery(el).offset().top)
					.data('timer', timer);
					$jquery(el).off('keypress', tmp_hide).on('keypress', tmp_hide).off('change', tmp_hide).on('change', tmp_hide)
						.css(options.css).addClass(options['class']);
					if (typeof options.onShow == 'function') options.onShow();
				}, 1);
				var originalCSS = '';
				if ($jquery(el).length > 0) originalCSS = $jquery(el)[0].style.cssText;
				// bind event to hide it
				function tmp_hide() { 
					if ($jquery('#global-tag-'+tagID).length <= 0) return;
					clearInterval($jquery('#global-tag-'+tagID).data('timer'));
					$jquery('#global-tag-'+tagID).remove(); 
					$jquery(el).off('keypress', tmp_hide).removeClass(options['class']); 
					if ($jquery(el).length > 0) $jquery(el)[0].style.cssText = originalCSS;
	...
	$jquery.fn.w2overlay = function (html, options) {
		var newOverlay = false;
		if (this.length == 0 || html == '') {
			$jquery('#w2ui-overlay').remove();
			return;
		}
		// insert overlay if it does not exist
		if ($jquery('#w2ui-overlay').length == 0) {
			$jquery('body').append('<div id="w2ui-overlay" class="w2ui-overlay" style="display: none;"><div></div></div>');
			newOverlay = true;
		}
		if (!$jquery.isPlainObject(options)) options = {};
		if (!$jquery.isPlainObject(options.css)) options.css = {};
		...
		// init
		var obj = this;
		var div = $jquery('#w2ui-overlay div');
		...
		var div = $jquery('#w2ui-overlay');
		div.css({
				display : 'block',
				left 	: ($jquery(obj).offset().left + options.left) + 'px',
				top 	: ($jquery(obj).offset().top + w2utils.getSize($jquery(obj), 'height') + 3 + options.top) + 'px'
			})
			.fadeIn('fast')
			.data('position', ($jquery(obj).offset().left) + 'x' + ($jquery(obj).offset().top + obj.offsetHeight))
		...
		setTimeout(function () {
			if (newOverlay) $jquery(document).on('click', hide);
			if (typeof options.onShow == 'function') options.onShow();
		}, 1);
	}
})();
...
(function () {
	...
	$jquery.fn.w2grid = function(method) {
		if (typeof method === 'object' || !method ) {
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				console.log('ERROR: The parameter "name" is required but not supplied in $jquery().w2grid().');
				return;
			}
			...
			$jquery.extend(object, { postData: {}, records: [], columns: [], searches: [], toolbar: {}, sortData: [], searchData: [], handlers: [] });
			if (object.onExpand != null) object.show.expandColumn = true;
			$jquery.extend(true, object.toolbar, toolbar);
			// reassign variables
			for (var p in columns)    	object.columns[p]    	= $jquery.extend({}, columns[p]);
			for (var p in columnGroups) object.columnGroups[p] 	= $jquery.extend({}, columnGroups[p]);
			for (var p in searches)   	object.searches[p]   	= $jquery.extend({}, searches[p]);
			for (var p in searchData) 	object.searchData[p] 	= $jquery.extend({}, searchData[p]);
			for (var p in sortData)    	object.sortData[p]  	= $jquery.extend({}, sortData[p]);
			for (var p in postData)   	object.postData[p]   	= $jquery.extend({}, postData[p]);
			...
			for (var r in records) {
				...
				object.records[r] = $jquery.extend({}, records[r]);
			}
			...
			// render if necessary
			if ($jquery(this).length != 0) {
				object.render($jquery(this)[0]);
			}
			// register new object
			w2ui[object.name] = object;
			return object;

		} else if (typeof $jquery(this).data('w2name') != 'undefined') {
			var obj = w2ui[$jquery(this).data('w2name')];
	...
	w2grid.prototype = {
		...
		add: function (record) {
			if (!$jquery.isArray(record)) record = [record];
			for (var o in record) {
		...
		set: function (recid, record) { // does not delete existing, but overrides on top of it
			...
			var record;
			$jquery.extend(this.records[ind], record);
			// refresh only that record
			var tr = $jquery('#grid_'+ this.name +'_rec_'+recid);
			if (tr.length != 0) {
				...
				while (true) {
					...
					$jquery(tr).find('#grid_'+ this.name +'_cell_'+ line + '_'+ j +' > div').html(field);
					// field
					j++;
		...
		localSort: function () {
			...
			this.records.sort(function (a, b) {
				...
				for (var s in obj.sortData) {
					...
					if (typeof aa == 'string') aa = $jquery.trim(aa.toLowerCase());
					if (typeof bb == 'string') bb = $jquery.trim(bb.toLowerCase());
		...
		select: function (recid) {
			...
			for (var a in arguments) {
				...
				$jquery('#grid_'+this.name +'_rec_'+ record.recid).addClass('w2ui-selected').data('selected', 'yes');
				$jquery('#grid_'+ this.name +'_cell_'+ i +'_select_check').attr('checked', true);
				selected++;
				// event after
				this.trigger($jquery.extend(eventData, { phase: 'after' }));
			} 
			// all selected?
			$jquery('#grid_'+ this.name +'_check_all').attr('checked', true);
			if ($jquery('#grid_'+ this.name +'_records').find('.grid_select_check[type=checkbox]').length == $jquery('#grid_'+ this.name +'_records').find('.grid_select_check[type=checkbox]:checked').length) {
				$jquery('#grid_'+ this.name +'_check_all').attr('checked', true);
			} else {
				$jquery('#grid_'+ this.name +'_check_all').removeAttr('checked');
			}
			return selected;
		},
		...
		unselect: function (recid) {
			...
			for (var a in arguments) {
				...
				$jquery('#grid_'+this.name +'_rec_'+ record.recid).removeClass('w2ui-selected').data('selected', '');
				if ($jquery('#grid_'+this.name +'_rec_'+ record.recid).length != 0) {
					$jquery('#grid_'+this.name +'_rec_'+ record.recid)[0].style.cssText = $jquery('#grid_'+this.name +'_rec_'+ record.recid).attr('custom_style');
				}
				$jquery('#grid_'+ this.name +'_cell_'+ i +'_select_check').removeAttr('checked');
				unselected++;
				// event after
				this.trigger($jquery.extend(eventData, { phase: 'after' }));
			} 
		...
		search: function (field, value) {
			...
			if (arguments.length == 0) {
				...
				for (var s in this.searches) {
					var search 	 = this.searches[s];
					var operator = $jquery('#grid_'+ this.name + '_operator_'+s).val();
					var value1   = $jquery('#grid_'+ this.name + '_field_'+s).val();
					var value2   = $jquery('#grid_'+ this.name + '_field2_'+s).val();
					if ((value1 != '' && value1 != null) || (typeof value2 != 'undefined' && value2 != '')) {
						...
						if (operator == 'between') {
							$jquery.extend(tmp, { value: [value1, value2] });
						} else {
							$jquery.extend(tmp, { value: value1 });
						}
						searchData.push(tmp);
					}
				}
			...
			if (arguments.length == 1 && $jquery.isArray(field)) {
				last_multi	= true;
				last_logic	= 'AND';
				for (var f in field) {
					var data   = field[f];
		...
		searchOpen: function () {
			...
			if ($jquery('#w2ui-global-searches').length == 0) {
				$jquery('body').append('<div id="w2ui-global-searches" class="w2ui-reset"> <div style=""></div> </div>');
			}
			$jquery('#w2ui-global-searches').css({
				left 	: $jquery('#grid_'+ this.name +'_search_all').offset().left + 'px',
				top 	: ($jquery('#grid_'+ this.name +'_search_all').offset().top
						   + w2utils.getSize($jquery('#grid_'+ this.name +'_search_all'),  'height')
						  ) + 'px'
			}).show().data('grid-name', this.name);
			$jquery('#w2ui-global-searches > div').html(this.getSearchesHTML())
			$jquery('#w2ui-global-searches > div').css({
				'-webkit-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
				'-moz-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
				'-ms-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
				'-o-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)'
			});
			setTimeout(function () {
				$jquery('#w2ui-global-searches > div').css({
			...
			setTimeout(function () {
				$jquery('#w2ui-global-searches').css({ 'box-shadow': '0px 2px 10px #777' });
			}, 350);
		},
		...
		searchClose: function () {
			...
			// slide down
			this.searchOpened = false;
			$jquery('#w2ui-global-searches').css('box-shadow', '0px 0px 0px white');
			$jquery('#w2ui-global-searches > div').css({
				'-webkit-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
				'-moz-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
				'-ms-transform': 'translate3d(0px, -'+ $jquery('#w2ui-global-searches').height() +'px, 0px)',
			...
			setTimeout(function () {
				$jquery('#w2ui-global-searches').hide();
				$jquery('#w2ui-global-searches > div').html('');
			}, 300);
		},
		...
		searchShowFields: function (el) {
			...
			for (var s = -1; s < this.searches.length; s++) {
				...
				html += '<tr style="cursor: default;" class="w2ui-tb-caption"'+
					'		onmouseover="$jquery(this).css(\'background-color\', \'#B4D5FE\'); " '+
					'		onmouseout="$jquery(this).css(\'background-color\', \'\');" '+
					'	'+ (this.isIOS ? 'onTouchStart' : 'onClick') +'="var obj = w2ui[\''+ this.name +'\']; '+
					'		if (\''+ search.type +'\' == \'list\' || \''+ search.type +'\' == \'enum\') {'+
					'			obj.last_search = \'\';'+
					'			obj.last_item = \'\';'+
					'			$jquery(\'#grid_'+ this.name +'_search_all\').val(\'\')'+
					'		}'+
					'		if (obj.last_search != \'\') { '+
					...
					'		}'+
					'		$jquery(\'#grid_'+ this.name +'_search_all\').attr(\'placeholder\', \''+ search.caption +'\');'+
					'		$jquery().w2overlay();">'+
			...
			html += "</table>";
			$jquery(el).w2overlay(html, { left: -15, top: 7 });
		},
		...
		searchReset: function () {
			...
			this.last_selected		= [];
			// reset on screen
			$jquery('#grid_'+ this.name +'_search_all').attr('placeholder', this.last_caption);
			$jquery('#w2ui-global-searches *[rel=search]').val(null);
			// -- clear all search field
			this.searchClose();
			$jquery('#grid_'+ this.name +'_search_all').attr('placeholder', this.last_caption);
			$jquery('#w2ui-global-searches *[rel=search]').val(null);
		...
		request: function (cmd, add_params, url, callBack) {
			...
			// append other params
			$jquery.extend(params, this.postData);
			$jquery.extend(params, add_params);
			// event before
			...
			this.request_xhr = $jquery.ajax({
				type		: 'POST',
				url			: url + (url.indexOf('?') > -1 ? '&' : '?') +'t=' + (new Date()).getTime(),
				data		: String($jquery.param(eventData.postData, false)).replace(/%5B/g, '[').replace(/%5D/g, ']'),
				dataType	: 'text',
				complete	: function (xhr, status) {
					...
					if (typeof eventData.data != 'undefined' && eventData.data != '') {
						var data = 'data = '+ eventData.data; 	// $jquery.parseJSON or $jquery.getJSON did not work because it expect perfect JSON data
						...
						if (typeof data['status'] != 'undefined' && data['status'] != 'success') {
							...
							setTimeout(function () {
								$jquery().w2popup('open', {
									...
									buttons : '<input type="button" value="Ok" onclick="$jquery().w2popup(\'close\');" style="width: 60px; margin-right: 5px;">'
								});
							}, 300);
						} else {
							if (cmd == 'get-records') $jquery.extend(obj, data);
					...
					obj.trigger($jquery.extend(eventData, { phase: 'after' }));
					obj.refresh();
					// call back
					if (typeof callBack == 'function') callBack();
				}
		...
		getChanged: function () {
			...
			for (var i=0; i<this.records.length; i++) {
				...
				for (var j=0; j<this.columns.length; j++) {
					var col = this.columns[j];
					if (col.hidden || col.field == 'recid' || $jquery('#grid_'+ this.name +'_edit_'+ i +'_'+ j).length == 0) continue;
					var newValue = $jquery('#grid_'+ this.name +'_edit_'+ i +'_'+ j).val();
				...
				if (!$jquery.isEmptyObject(tmp)) {
					saveData.push($jquery.extend({}, { recid: this.records[i].recid }, tmp));
				}
			}
			return saveData;
		},
		...
		doSave: function () {
			...
			if (this.url != '') {
				...
				$jquery('#grid_'+ this.name + ' .w2ui-editable').removeClass('changed');
				this.selectNone();
				// event after
				this.trigger($jquery.extend(eventData, { phase: 'after' }));
			}
		...
		doEdit: function (type, el, event) {
			var recid  = $jquery(el).attr('recid');
			...
			switch (type) {
				...
				case 'blur':
					...
					var oldValue = record[$jquery(el).attr('field')];
					if (typeof oldValue == 'undefined') oldValue = '';
					if ($jquery(el).val() != oldValue) {
						var eventData = this.trigger({ phase: 'before', type: 'change', target: el.id, recid: recid });
						if (eventData.stop === true) return false;
						$jquery(el).addClass('changed');
						this.trigger($jquery.extend(eventData, { phase: 'after' }));
					} else {
						$jquery(el).removeClass('changed');
					}
					break;
				case 'keyup':
					if ($jquery(el).data('stop') === true) {
						$jquery(el).data('stop', false);
						break;
					}
					if (event.keyCode == 40 || event.keyCode == 13) {
						var newEl = $jquery('#grid_'+ this.name + '_edit_'+ (parseInt($jquery(el).attr('line')) + 1) +'_'+ $jquery(el).attr('column'));
						if (newEl.length > 0) {
							newEl[0].select();
							newEl[0].focus();
						}
		...
		doDelete: function (force) {
			...
			if (this.msgDelete != '' && !force) {
				$jquery().w2popup({
					...
					buttons : '<input type="button" value="No" onclick="$jquery().w2popup(\'close\');" style="width: 60px; margin-right: 5px;">'+
							  '<input type="button" value="Yes" onclick="w2ui[\''+ this.name +'\'].doDelete(true); $jquery().w2popup(\'close\');" style="width: 60px;">'
				});
				return;
			}
			// call delete script
		...
		doClick: function (recid, event) {
			...
			if (this.keyboard) {
				...
				var grid_keydown = function (event) {
					...
					if (event.target.tagName.toLowerCase() == 'body') {
						...
						if (sel.length == 1) {
							...
							var sTop    = parseInt($jquery('#grid_'+ obj.name +'_records').prop('scrollTop'));
							var sHeight = parseInt($jquery('#grid_'+ obj.name +'_records').height());
							if (event.keyCode == 38) { // up
								if (ind > 0) {
									...
									// scroll into view
									var rTop 	= parseInt($jquery('#grid_'+ obj.name +'_rec_'+ obj.records[ind].recid)[0].offsetTop);
									var rHeight = parseInt($jquery('#grid_'+ obj.name +'_rec_'+ obj.records[ind].recid).height());
									if (rTop < sTop) {
										$jquery('#grid_'+ obj.name +'_records').prop('scrollTop', rTop - rHeight * 0.7);
										obj.last_scrollTop = $jquery('#grid_'+ obj.name +'_records').prop('scrollTop');
									}
								}
								event.preventDefault();
							}
							if (event.keyCode == 40) { // down
								if (ind + 1 < obj.records.length) {
									...
									if (rTop + rHeight > sHeight + sTop) {
										$jquery('#grid_'+ obj.name +'_records').prop('scrollTop', -(sHeight - rTop - rHeight) + rHeight * 0.7);
										obj.last_scrollTop = $jquery('#grid_'+ obj.name +'_records').prop('scrollTop');
									}
								}
								event.preventDefault();
				...
				$jquery(window).off('keydown').on('keydown', grid_keydown);
			}
			if (this.getSelection().length > 0) this.toolbar.enable('delete-selected'); else this.toolbar.disable('delete-selected');
			// remember last selected
			this.last_selected	 = this.getSelection();
		...
		doExpand: function (recid) {
			var expanded = $jquery('#grid_'+this.name +'_rec_'+ recid).attr('expanded');
			if (expanded != 'yes') {
				var tmp = 1 + (this.show.lineNumbers ? 1 : 0) + (this.show.selectColumn ? 1 : 0);
				var addClass = ($jquery('#grid_'+this.name +'_rec_'+ recid).hasClass('w2ui-odd') ? 'w2ui-odd' : 'w2ui-even');
				$jquery('#grid_'+this.name +'_rec_'+ recid).after(
			...
			if (eventData.stop === true) { 	$jquery('#grid_'+this.name +'_rec_'+ recid +'_expaned_row').remove(); return false; }
			// default action
			if (expanded == 'yes') {
				$jquery('#grid_'+this.name +'_rec_'+ recid).attr('expanded', '');
				$jquery('#grid_'+this.name +'_rec_'+ recid +'_expaned_row').remove();
				$jquery('#grid_'+this.name +'_cell_'+ this.getIndex(recid) +'_expand div').html('+');
			} else {
				$jquery('#grid_'+this.name +'_rec_'+ recid).attr('expanded', 'yes')
				$jquery('#grid_'+this.name +'_cell_'+ this.getIndex(recid) +'_expand div').html('-');
				$jquery('#grid_'+this.name +'_rec_'+ recid +'_expaned_row').show();
			}
			this.trigger($jquery.extend(eventData, { phase: 'after' }));
			this.resize();
		},
		...
		resize: function (width, height) {
			...
			if (!this.box || $jquery(this.box).data('w2name') != this.name) return;
			// determine new width and height
			this.width  = typeof width != 'undefined' && width != null ? parseInt(width) : $jquery(this.box).width();
			this.height = typeof height != 'undefined' && height != null ? parseInt(height) : $jquery(this.box).height();
			$jquery(this.box).width(width).height(height);
			// if blank
		...
		refresh: function () {
			...
			// -- header
			if (this.show.header) {
				$jquery('#grid_'+ this.name +'_header').html(this.header +'&nbsp;').show();
			} else {
				$jquery('#grid_'+ this.name +'_header').hide();
			}
			// -- toolbar
			if (this.show.toolbar) {
				$jquery('#grid_'+ this.name +'_toolbar').show();
				// refresh toolbar only once
				if (typeof this.toolbar == 'object') {
					this.toolbar.refresh();
					$jquery('#grid_'+ this.name +'_search_all').val(this.last_search);
				}
			} else {
				$jquery('#grid_'+ this.name +'_toolbar').hide();
			}
			...
			if (this.last_multi) {
				$jquery('#grid_'+ this.name +'_search_all').attr('placeholder', 'Multi Fields');
			} else {
				$jquery('#grid_'+ this.name +'_search_all').attr('placeholder', this.last_caption);
			}
			...
			if (this._focus_when_refreshed === true) {
				setTimeout(function () {
					var s = $jquery('#grid_'+ obj.name +'_search_all');
			...
			$jquery('#grid_'+ this.name +'_body').html(bodyHTML);
			// init editable
			$jquery('#grid_'+ this.name + '_records .w2ui-editable').each(function (index, el) {
				var column = obj.columns[$jquery(el).attr('column')];
				$jquery(el).w2field(column.editable);
			});

			// -- summary
			if (this.summary != '') {
				$jquery('#grid_'+ this.name +'_summary').html(this.summary).show();
			} else {
				$jquery('#grid_'+ this.name +'_summary').hide();
			}
			...
			if (this.show.footer) {
				...
				$jquery('#grid_'+ this.name +'_footerR').html(pageCountDsp);
				$jquery('#grid_'+ this.name +'_footerC').html(pages);
				$jquery('#grid_'+ this.name +'_footer').show();
			} else {
				$jquery('#grid_'+ this.name +'_footer').hide();
			}
			...
			// show/hide clear search link
 			if (this.searchData.length > 0) {
				$jquery('#grid_'+ this.name +'_searchClear').show();
			} else {
				$jquery('#grid_'+ this.name +'_searchClear').hide();
			}
			// all selected?
			$jquery('#grid_'+ this.name +'_check_all').attr('checked', true);
			if ($jquery('#grid_'+ this.name +'_records').find('.grid_select_check[type=checkbox]').length == $jquery('#grid_'+ this.name +'_records').find('.grid_select_check[type=checkbox]:checked').length) {
		...
		render: function (box) {
			...
			if (!this.box) return;
			$jquery('#w2ui-global-searches').remove(); // show searches from previous grid
			...
			if (eventData.stop === true) return false;
			// insert Elements
			$jquery(this.box).data('w2name', this.name).html(
			...
			// init toolbar
			this.initToolbar();
			if (this.toolbar != null) this.toolbar.render($jquery('#grid_'+ this.name +'_toolbar')[0]);
			// init footer
			$jquery('#grid_'+ this.name +'_footer').html(
		...
		initToolbar: function () {
			...
			if (typeof this.toolbar['render'] == 'undefined') {
				...
				this.toolbar.items = [];
				this.toolbar = $jquery().w2toolbar($jquery.extend(true, {}, this.toolbar, { name: this.name +'_toolbar', owner: this }));
				...
				if (this.show.toolbarColumns) {
					...
					for (var c in this.columns) {
						col_html += "<tr style=\"cursor: default;\" "+
							"	onmouseover=\"$jquery(this).addClass('w2ui-selected');\" onmouseout=\"$jquery(this).removeClass('w2ui-selected');\">"+
						...
						case 'select-columns':
							for (var c in obj.columns) {
								if (obj.columns[c].hidden) {
									$jquery("#grid_"+ obj.name +"_column_"+ c + "_check").removeAttr('checked');
								} else {
									$jquery("#grid_"+ obj.name +"_column_"+ c + "_check").attr('checked', true);
								}
							}
							// restore sizes
							for (var c in obj.columns) {
		...
		initSearches: function () {
			...
			for (var s in this.searches) {
				...
				switch (search.type) {
					case 'alphaNumeric':
					case 'text':
						$jquery('#grid_'+ this.name +'_operator_'+s).val('begins with');
						break;
					...
					case 'money':
					case 'date':
						$jquery('#grid_'+ this.name +'_field_'+s).w2field(search.type);
						$jquery('#grid_'+ this.name +'_field2_'+s).w2field(search.type);
						break;
					...
					case 'list':
						...
						for (var i in search.items) {
							if ($jquery.isPlainObject(search.items[i])) {
						...
						$jquery('#grid_'+ this.name +'_field_'+s).html(options);
						break;
				}
				if (sdata != null) {
					$jquery('#grid_'+ this.name +'_operator_'+ s).val(sdata.operator).trigger('change');
					if (!$jquery.isArray(sdata.value)) {
						if (typeof sdata.value != 'udefined') $jquery('#grid_'+ this.name +'_field_'+ s).val(sdata.value).trigger('change');						
					} else {
						$jquery('#grid_'+ this.name +'_field_'+ s).val(sdata.value[0]).trigger('change');
						$jquery('#grid_'+ this.name +'_field2_'+ s).val(sdata.value[1]).trigger('change');
					}
				}
			}
			// add on change event
			$jquery('#w2ui-global-searches *[rel=search]').on('keypress', function (evnt) {
		...
		initResize: function () {
			...
			$jquery('#grid_'+ this.name + ' .w2ui-resizer')
				.off('mousedown')
				.off('click')
				...
				.on('mousedown', function (event) {
					...
					obj.tmp_col = $jquery(this).attr('name');
					event.stopPropagation();
					event.preventDefault();
					// fix sizes
					for (var c in obj.columns) {
					...
					var mouseUp = function (event) {
						...
						$jquery(window).off('mouseup', mouseUp);
						obj.resizeRecords();
					}
					$jquery(window).on('mousemove', mouseMove);
					$jquery(window).on('mouseup', mouseUp);
				})
		...
		resizeBoxes: function () {
			// elements
			var main 		= $jquery('#grid_'+ this.name);
			var header 		= $jquery('#grid_'+ this.name +'_header');
			var toolbar 	= $jquery('#grid_'+ this.name +'_toolbar');
			var summary		= $jquery('#grid_'+ this.name +'_summary');
			var footer		= $jquery('#grid_'+ this.name +'_footer');
			var body		= $jquery('#grid_'+ this.name +'_body');
			var columns 	= $jquery('#grid_'+ this.name +'_columns');
			var records 	= $jquery('#grid_'+ this.name +'_records');
		...
		resizeRecords: function () {
			...
			$jquery('.grid_'+ this.name + '_empty_record').remove();
			...
			if (!this.fixedBody) {
				...
				var calculatedHeight = $jquery('#grid_'+ this.name +'_records > table:first-child').height() + columns.height();
			...
			// check overflow
			if (body.height() - columns.height() < $jquery(records).find(':first-child').height()) var bodyOverflowY = true; else bodyOverflowY = false;
			if (body.width() < $jquery(records).find(':first-child').width())   var bodyOverflowX = true; else bodyOverflowX = false;
			if (bodyOverflowX || bodyOverflowY) {
				...
				$jquery('#grid_'+ this.name +'_cell_header_last').show();
			} else {
				...
				$jquery('#grid_'+ this.name +'_cell_header_last').hide();
			}
			if (this.show.emptyRecords && !bodyOverflowY) {
				...
				$jquery('#grid_'+ this.name +'_records > table').append(html);
			}
			if (body.length > 0) {
			...
			for (var i=0; i<this.columns.length; i++) {
				var col = this.columns[i];
				if (col.hidden) continue;
				var tmp = $jquery('#grid_'+ this.name +'_cell_header_'+ i + ' > div:first-child');
				...
				for (var j=startWith; j<1000; j++) {
					...
					var cell = $jquery('#grid_'+ this.name+'_cell_'+ j +'_'+ i + ' > div:first-child');
		...
		getSearchesHTML: function () {
			...
			for (var i = 0; i < this.searches.length; i++) {
				...
				if (s.type == 'int' || s.type == 'float' || s.type == 'date') {
					var operator =  '<select id="grid_'+ this.name +'_operator_'+i+'" onchange="var el = $jquery(\'#grid_'+ this.name + '_range_'+ i +'\'); '+
						'					if ($jquery(this).val() == \'is\') el.hide(); else el.show();">'+
						'	<option value="is">is</option>'+
						...
						for (var i = startWith, di = 0, ri = 0; ri < this.recordsPerPage && i < this.records.length; i++) {
							...
							while (true) {
								...
								if (this.fixedRecord) {
									...
									if ($jquery.isPlainObject(col.editable)) {
		...
		showStatus: function (status) {
			...
			setTimeout(function () {
				var lock 	= $jquery('#grid_'+ obj.name +'_lock');
				var status 	= $jquery('#grid_'+ obj.name +'_status');
				status.data('old_opacity', status.css('opacity')).css('opacity', '0').show();
				lock.data('old_opacity', lock.css('opacity')).css('opacity', '0').show();
				setTimeout(function () {
					var left 	= ($jquery(obj.box).width()  - w2utils.getSize(status, 'width')) / 2;
					var top 	= ($jquery(obj.box).height() * 0.9 - w2utils.getSize(status, 'height')) / 2;
					lock.css({
		...
		hideStatus: function () {
			var obj = this;
			setTimeout(function () {
				$jquery('#grid_'+ obj.name +'_lock').remove();
				$jquery('#grid_'+ obj.name +'_status').remove();
			}, 25);
		}
	}

	$jquery.extend(w2grid.prototype, $jquery.w2event);
})();
/************************************************************************
*   Library: Web 2.0 UI for jQuery (using prototypical inheritance)
*   - Following objects defined
* 		- w2ui.w2layout - layout widget
*		- $jquery.w2layout	- jQuery wrapper
...
(function () {
	...
	$jquery.fn.w2layout = function(method) {
		if (typeof method === 'object' || !method ) {
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				$jquery.error('The parameter "name" is required but not supplied in $jquery().w2layout().');
				return;
			}
			if (typeof w2ui[method.name] != 'undefined') {
				$jquery.error('The parameter "name" is not unique. There are other objects already created with the same name (obj: '+ method.name +').');
				return;			
			}
			var panels = method.panels;
			var object = new w2layout(method);
			$jquery.extend(object, { handlers: [], panels: [] });
			for (var p in panels) { object.panels[p] = $jquery.extend({}, w2layout.prototype.panel, panels[p]); }
			if ($jquery(this).length > 0) {
				$jquery(this).data('w2name', object.name);
				object.render($jquery(this)[0]);
			}
			w2ui[object.name] = object;		
			return object;		
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2layout' );
		}    
	};
	...
	w2layout.prototype = {
		...
		add: function (options) {
			if (!$jquery.isArray(options)) options = [options];
			for (var o in options) {
				var panel = $jquery.extend({}, panel, options[o]);
		...
		content: function (panel, data, transition) {
			...
			if ($jquery('#layout_'+ this.name + '_panel2_'+ p.type).length > 0) return false;
			$jquery('#layout_'+ this.name + '_panel_'+ p.type).scrollTop(0);
						...
						if (transition != null && transition != '' && typeof transition != 'undefined') {
							...
							if (String(transition).substr(0, 5) == 'slide') {
								...
								w2utils.transition(pan.find('#'+ nm +'_old')[0], pan.find('#'+ nm)[0], transition, function () {
									// clean up
									var pan = $jquery('#layout_'+ obj.name + '_panel_'+ p.type +'_trans');
									if (pan.length > 0) {
								...
								$jquery('#layout_'+ this.name + '_panel_'+ p.type).before('<div id="layout_'+ this.name + '_panel2_'+ p.type + '">'+ data +'</div>');					
								$jquery('#layout_'+ this.name + '_panel2_'+ p.type)[0].style.cssText = $jquery('#layout_'+ this.name + '_panel_'+ p.type)[0].style.cssText;
								if (typeof data == 'object') { 
									data.render($jquery('#layout_'+ this.name + '_panel2_'+ p.type)[0]); 
								}
								var div1 = $jquery('#layout_'+ this.name + '_panel2_'+ p.type)[0];
								var div2 = $jquery('#layout_'+ this.name + '_panel_'+ p.type)[0];
								var obj  = this;
								w2utils.transition(div2, div1, transition, function () {
									// clean up
									$jquery('#layout_'+ obj.name + '_panel_'+ p.type).remove();
									$jquery('#layout_'+ obj.name + '_panel2_'+ p.type).attr('id', 'layout_'+ obj.name + '_panel_'+ p.type);
									p.content = data;
									// IE Hack
									if (window.navigator.userAgent.indexOf('MSIE')) setTimeout(function () { obj.resize(); }, 100);
								});
		...
		load: function (panel, url, transition, onLoad) {
			var obj = this;
			$jquery.get(url, function (data, status, object) {
		...
		show: function (panel, immediate) {
			...
			p.hidden = false;
			if (immediate === true) {
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_'+panel).css({ 'opacity': '1' });	
				if (p.resizabled) $jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_'+panel).show();
				this.trigger($jquery.extend(eventData, { phase: 'after' }));	
				this.resize();
			} else {			
				var obj = this;
				if (p.resizabled) $jquery('#layout_'+ obj.name +' #layout_'+ obj.name +'_splitter_'+panel).show();
				// resize
				$jquery('#layout_'+ obj.name +' #layout_'+ obj.name +'_panel_'+panel).css({ 'opacity': '0' });	
				$jquery('#layout_'+ this.name +' .w2ui-panel').css({
					'-webkit-transition': '.2s',
					'-moz-transition'	: '.2s',
					'-ms-transition'	: '.2s',
					'-o-transition'		: '.2s'
				...
				// show
				setTimeout(function() {
					$jquery('#layout_'+ obj.name +' #layout_'+ obj.name +'_panel_'+panel).css({ 'opacity': '1' });	
				}, 250);
				// clean
				setTimeout(function () { 
					$jquery('#layout_'+ obj.name +' .w2ui-panel').css({
		...
		hide: function (panel, immediate) {
			...
			p.hidden = true;		
			if (immediate === true) {
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_'+panel).css({ 'opacity': '0'	});
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_'+panel).hide();
				this.trigger($jquery.extend(eventData, { phase: 'after' }));	
				this.resize();
			} else {
				var obj = this;
				$jquery('#layout_'+ obj.name +' #layout_'+ obj.name +'_splitter_'+panel).hide();
				// hide
				$jquery('#layout_'+ this.name +' .w2ui-panel').css({
					'-webkit-transition': '.2s',
					'-moz-transition'	: '.2s',
		...
		set: function (panel, options) {
			...
			$jquery.extend(this.panels[obj], options);
			this.refresh(panel);
			return true;		
		},
		...
		render: function (box) {
			...
			if (this.get('main') == null) this.panels.push( $jquery.extend({}, w2layout.prototype.panel, { type: 'main'}) );
			if (this.get('css') == null)  this.panels.push( $jquery.extend({}, w2layout.prototype.panel, { type: 'css'}) );
			var html = '<div id="layout_'+ this.name +'" class="w2ui-layout" style="position: absolute; overflow: hidden; '+ this.style +'"></div>';
			$jquery(this.box).html(html);
			// create all panels
			...
			for (var t in tmp) {
				var html =  '<div id="layout_'+ this.name + '_panel_'+ tmp[t] +'" class="w2ui-panel"'+
							...
							'</div>';
				$jquery('#layout_'+ this.name +'').append(html);
			}
			$jquery('#layout_'+ this.name +'').append('<style id="layout_'+ this.name + '_panel_css" style="position: absolute; top: 10000px;">'+ this.css +'</style>');		
			// process event
			this.trigger($jquery.extend(eventData, { phase: 'after' }));	
			// reinit events
			this.refresh();
		...
		refresh: function (panel) {
			...
			if (panel != null && typeof panel != 'undefined') {
				...
				var el = $jquery('#layout_'+ this.name +' #layout_' +this.name +'_panel_'+panel).css({
				...
				if (typeof p.content == 'object' && p.content.render) {
					p.content.render($jquery('#layout_'+ this.name +' #layout_'+ this.name + '_panel_'+ p.type)[0]);
				} else {
					$jquery('#layout_'+ this.name +' #layout_'+ this.name + '_panel_'+ p.type).html(p.content);
				}
			} else {
				if ($jquery('#layout_'+ this.name +' #layout_' +this.name +'_panel_main').length <= 0) {
					this.render();
					return;
				}
				this.resize();
		...
		resize: function (width, height) {
			...
			// layout itself
			this.width  = parseInt($jquery(this.box).width());
			this.height = parseInt($jquery(this.box).height());
			...
			if (ptop != null && ptop.hidden != true) {
				...
				var w = this.width;
				var h = ptop.size;
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_top').css({
					'display': 'block',
					'left': l + 'px',
					'top': t + 'px',
					'width': w + 'px',
				...
				if (ptop.resizable) {
					t = ptop.size;
					h = this.spacer;
					$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_top').show().css({
						'display': 'block',
						'left': l + 'px',
						'top': t + 'px',
						'width': w + 'px',
				...
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_top').hide();
			}
			// left if any
			if (pleft != null && pleft.hidden != true) {
				...
				var e = $jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_left');
				if (window.navigator.userAgent.indexOf('MSIE') > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_left').css({
					'display': 'block',
					'left': l + 'px',
					'top': t + 'px',
					'width': w + 'px',
				...
				if (pleft.resizable) {
					l = pleft.size;
					w = this.spacer;
					$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_left').show().css({
						'display': 'block',
						'left': l + 'px',
						'top': t + 'px',
						'width': w + 'px',
				...
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_left').hide();
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_left').hide();
			}
			// right if any
			if (pright != null && pright.hidden != true) {
				...
				var h = this.height - (stop ? ptop.size + (ptop.resizable ? this.spacer : this.padding) : 0) - 
									  (sbottom ? pbottom.size + (pbottom.resizable ? this.spacer : this.padding) : 0);
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_right').css({
					'display': 'block',
					'left': l + 'px',
					'top': t + 'px',
					'width': w + 'px',
				...
				if (pright.resizable) {
					l = l - this.spacer;
					w = this.spacer;
					$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_right').show().css({
						'display': 'block',
						'left': l + 'px',
						'top': t + 'px',
						'width': w + 'px',
				...
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_right').hide();
			}
			// bottom if any
			if (pbottom != null && pbottom.hidden != true) {
				...
				var w = this.width;
				var h = pbottom.size;
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_bottom').css({
					'display': 'block',
					'left': l + 'px',
					'top': t + 'px',
					'width': w + 'px',
				...
				if (pbottom.resizable) {
					t = t - this.spacer;
					h = this.spacer;
					$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_bottom').show().css({
						'display': 'block',
						'left': l + 'px',
						'top': t + 'px',
						'width': w + 'px',
				...
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_bottom').hide();
			}
			// main - always there
			...
			var e = $jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_main');
			if (window.navigator.userAgent.indexOf('MSIE') > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
			$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_main').css({
				'display': 'block',
				'left': l + 'px',
				'top': t + 'px',
				'width': w + 'px',
			...
			if (pprev != null && pprev.hidden != true) {
				...
				var w = this.width  - (sleft ? pleft.size + (pleft.resizable ? this.spacer : this.padding) : 0) - 
									  (sright ? pright.size + (pright.resizable ? this.spacer : this.padding): 0);
				var h = pprev.size;
				var e = $jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_preview');
				if (window.navigator.userAgent.indexOf('MSIE') > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight) w += 17; // IE hack
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_preview').css({
					'display': 'block',
					'left': l + 'px',
					'top': t + 'px',
					'width': w + 'px',
				...
				if (pprev.resizable) {
					t = t - this.spacer;
					h = this.spacer;
					$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_splitter_preview').show().css({
						'display': 'block',
						'left': l + 'px',
						'top': t + 'px',
						'width': w + 'px',
				...
				$jquery('#layout_'+ this.name +' #layout_'+ this.name +'_panel_preview').hide();
			}
		...
		startResize: function (type, evnt) {
			...
			if (type == 'left' || type == 'right') {
				this.tmp_value = parseInt($jquery('#layout_'+ this.name + '_splitter_'+ type)[0].style.left);
			}
			if (type == 'top' || type == 'preview' || type == 'bottom') {
				this.tmp_value = parseInt($jquery('#layout_'+ this.name + '_splitter_'+ type)[0].style.top);
			}
		},
		...
		doResize: function (evnt) {
			...
			var p = $jquery('#layout_'+ this.name + '_splitter_'+ this.tmp_resizing);
		...
		stopResize: function (evnt) {
			...
			this.resize();
			$jquery('#layout_'+ this.name + '_splitter_'+ this.tmp_resizing).removeClass('active');
			delete this.tmp_resizing;
		}		
	}
	
	$jquery.extend(w2layout.prototype, $jquery.w2event);
})();
/************************************************************************
*   Library: Web 2.0 UI for jQuery (using prototypical inheritance)
*   - Following objects defined
* 		- w2popup 	- popup widget
*		- $jquery.w2popup	- jQuery wrapper
*   - Dependencies: jQuery, w2utils
* 
************************************************************************/
...
(function () {
	...
	$jquery.fn.w2popup = function(method, options) {	
		if (typeof method  == 'undefined') {
			options = {};
			method  = 'open';
		}
		if ($jquery.isPlainObject(method)) {
			options = method;		
			method  = 'open';
		}
		...
		var dlgOptions = {};
		if ($jquery(this).length > 0 ) {
			if ($jquery(this).find('div[rel=title]').length > 0) 		dlgOptions['title'] 	= $jquery(this).find('div[rel=title]').html();
			if ($jquery(this).find('div[rel=body]').length > 0) 		dlgOptions['body'] 		= $jquery(this).find('div[rel=body]').html();
			if ($jquery(this).find('div[rel=buttons]').length > 0) 	dlgOptions['buttons'] 	= $jquery(this).find('div[rel=buttons]').html();
			if (parseInt($jquery(this).css('width')) != 0)  dlgOptions['width']  = parseInt($jquery(this).css('width'));
			if (parseInt($jquery(this).css('height')) != 0) dlgOptions['height'] = parseInt($jquery(this).css('height'));
			if (String($jquery(this).css('overflow')) != 'undefined') dlgOptions['overflow'] = $jquery(this).css('overflow');
			}
		// show popup
		return window.w2popup[method]($jquery.extend({}, dlgOptions, options));
	};
	...
	window.w2popup = {	
		...
		open: function (options) {
			...
			var old_options = $jquery('#w2ui-popup').data('options');
			...
			if ($jquery('#w2ui-popup').length == 0) {
				// output message
				window.w2popup.lock($jquery.extend({}, options, {
					onMouseDown: options.modal ? function () {
						$jquery('#w2ui-lock').css({ 
				...
				if (options.title != '') { 
					msg +='<div class="w2ui-msg-title">'+
						  (options.showClose ? '<div class="w2ui-msg-button w2ui-msg-close" onclick="$jquery().w2popup(\'close\'); event.stopPropagation();">Close</div>' : '')+ 
						  (options.showMax ? '<div class="w2ui-msg-button w2ui-msg-max" onclick="$jquery().w2popup(\'max\')">Max</div>' : '') + 
							  options.title +
						  '</div>'; 
				}
				...
				setTimeout(function () {
					$jquery('#w2ui-popup .w2ui-box2').hide();
					$jquery('#w2ui-popup').css({ 
				...
				if (typeof old_options == 'undefined' || old_options['width'] != options['width'] || old_options['height'] != options['height']) {
					$jquery('#w2ui-panel').remove();
					window.w2popup.resize(options.width, options.height);
				}
				// show new items
				$jquery('#w2ui-popup .w2ui-box2 > .w2ui-msg-body').html(options.body).css('overflow', options.overflow);
				$jquery('#w2ui-popup .w2ui-msg-buttons').html(options.buttons);
				$jquery('#w2ui-popup .w2ui-msg-title').html(
					  (options.showClose ? '<div class="w2ui-msg-button w2ui-msg-close" onclick="$jquery().w2popup(\'close\')">Close</div>' : '')+ 
					  (options.showMax ? '<div class="w2ui-msg-button w2ui-msg-max" onclick="$jquery().w2popup(\'max\')">Max</div>' : '') + 
					  options.title);
				// transition
				var div_old = $jquery('#w2ui-popup .w2ui-box1')[0];
				var div_new = $jquery('#w2ui-popup .w2ui-box2')[0];
				w2utils.transition(div_old, div_new, options.transition);
				div_new.className = 'w2ui-box1';
				div_old.className = 'w2ui-box2';	
				$jquery(div_new).addClass('w2ui-current-box');		
				// remove max state
				$jquery('#w2ui-popup').data('prev-size', null);
				// call event onChange
				setTimeout(function () {
			...
			// save new options
			$jquery('#w2ui-popup').data('options', options);	
		...
		close: function (options) {
			var options = $jquery.extend({}, $jquery('#w2ui-popup').data('options'), options);
			if (typeof options.onBeforeClose == 'function') {
				if (options.onBeforeClose() === false) return;
			}
			$jquery('#w2ui-popup, #w2ui-panel').css({ 
				'-webkit-transition': options.speed +'s opacity, '+ options.speed +'s -webkit-transform', 
		...
		max: function () {
			var options = $jquery('#w2ui-popup').data('options');
			// if panel is out - remove it
			$jquery('#w2ui-panel').remove();
			// resize
			if ($jquery('#w2ui-popup').data('prev-size')) {
				var size = String($jquery('#w2ui-popup').data('prev-size')).split(':');
				$jquery('#w2ui-popup').data('prev-size', null);
				window.w2popup.resize(size[0], size[1], function () {
					if (typeof options.onMax == 'function') options.onMax();
				});
			} else {
				$jquery('#w2ui-popup').data('prev-size', $jquery('#w2ui-popup').css('width')+':'+$jquery('#w2ui-popup').css('height'));
		...
		get: function () {
			return $jquery('#w2ui-popup').data('options');
		},
		
		clear: function() {
			$jquery('#w2ui-popup .w2ui-msg-title').html('');
			$jquery('#w2ui-popup .w2ui-msg-body').html('');
			$jquery('#w2ui-popup .w2ui-msg-buttons').html('');
		},
		...
		load: function (options) {
			if (String(options.url) == 'undefined') {
				$jquery.error('The url parameter is empty.');
				return;
			}
			...
			// load url
			var html = $jquery('#w2ui-popup').data(url);
			if (typeof html != 'undefined') {
				popup(html, selector);
			} else {
				$jquery.post(url, function (data, status, obj) {
					popup(obj.responseText, selector);
					$jquery('#w2ui-popup').data(url, obj.responseText); // remember for possible future purposes
				});
			}
			function popup(html, selector) {
				$jquery('body').append('<div id="w2ui-tmp" style="display: none">'+ html +'</div>');
				if (typeof selector != 'undefined' && $jquery('#w2ui-tmp #'+selector).length > 0) {
					$jquery('#w2ui-tmp #'+ selector).w2popup(options);
				} else {
					$jquery('#w2ui-tmp > div').w2popup(options);
				}
				// link styles
				if ($jquery('#w2ui-tmp > style').length > 0) {
					var style = $jquery('<div>').append($jquery('#w2ui-tmp > style').clone()).html();
					if ($jquery('#w2ui-screenPopup #div-style').length == 0) {
						$jquery('#w2ui-screenPopup').append('<div id="div-style" style="position: absolute; left: -100; width: 1px"></div>');
					}
					$jquery('#w2ui-screenPopup #div-style').html(style);
				}
				$jquery('#w2ui-tmp').remove();
			}
		},
		
		message: function (options) {
			$jquery().w2tag(); // hide all tags
			if (parseInt(options.width) < 10)  options.width  = 10;
			if (parseInt(options.height) < 10) options.height = 10;
			if (typeof options.hideOnClick == 'undefined') options.hideOnClick = true;

			if ($jquery('#w2ui-popup .w2ui-popup-message').length == 0) {
				var pwidth = parseInt($jquery('#w2ui-popup').width());
				$jquery('#w2ui-popup .w2ui-box1 .w2ui-msg-body')
					.append('<div class="w2ui-popup-message" style="position: absolute; top: 0px; display: none; '+
								...
								(options.hideOnClick === true ? 'onclick="$jquery().w2popup(\'message\');"' : '') + '>'+
							'</div>');
				$jquery('#w2ui-popup .w2ui-box1 .w2ui-msg-body').prop('scrollTop', 0);	
			} else {
				if (typeof options.width  == 'undefined') options.width  = w2utils.getSize($jquery('#w2ui-popup .w2ui-popup-message'), 'width');
				if (typeof options.height == 'undefined') options.height = w2utils.getSize($jquery('#w2ui-popup .w2ui-popup-message'), 'height');
			}
			var display = $jquery('#w2ui-popup .w2ui-popup-message').css('display');
			$jquery('#w2ui-popup .w2ui-popup-message').css({ 
			...
			if (display == 'none') {
				$jquery('#w2ui-popup .w2ui-popup-message').show().html(options.html);
				$jquery('#w2ui-popup .w2ui-msg-buttons').fadeOut('slow');
				if (typeof options.onOpen == 'function') options.onOpen();
			} else {
				$jquery('#w2ui-popup .w2ui-msg-buttons').fadeIn('slow');		
			}
			...
			setTimeout(function () {
				if (display != 'none') {
					$jquery('#w2ui-popup .w2ui-popup-message').remove();		
		...
		lock: function (options) {
			if ($jquery('#w2ui-lock').length > 0) return false;
			var options = $jquery.extend({}, { 'onUnlock': null, 'onMouseDown': null, 'onMouseUp': null }, options);
			// show element
			$jquery('body').append('<div id="w2ui-lock" onmousewheel="event.stopPropagation(); event.preventDefault()"'+
			...
			//$jquery('body').data('_old_overflow', $jquery('body').css('overflow')).css({ 'overflow': 'hidden' });		
			// lock events
			if (typeof options.onMouseDown == 'function') { 
				$jquery('#w2ui-lock').bind('mousedown', options.onMouseDown); 
			}
			if (typeof options.onMouseUp == 'function') { 
				$jquery('#w2ui-lock').bind('mouseup', options.onMouseUp); 
			}
			return true;
		},
		...
		unlock: function (options) {
			...
			//$jquery('body').css({ 'overflow': $jquery('body').data('_old_overflow') });		
			setTimeout(function () { 
				$jquery('#w2ui-lock').remove(); 
		...
		initMove: function () {
			var obj = this;
			$jquery('#w2ui-popup .w2ui-msg-title')
			...
			$jquery('#w2ui-popup .w2ui-msg-body')
				.on('mousemove', function () { obj.doMove.apply(obj, arguments); })
				.on('mouseup',   function () { obj.stopMove.apply(obj, arguments); });
			$jquery('#w2ui-lock')
				.on('mousemove', function () { obj.doMove.apply(obj, arguments); })
		...
		resize: function (width, height, callBack) {
			...
			if (parseInt($jquery(window).width())  - 10 < parseInt(width))  width  = parseInt($jquery(window).width())  - 10;
			if (parseInt($jquery(window).height()) - 10 < parseInt(height)) height = parseInt($jquery(window).height()) - 10;
			var top  = ((parseInt($jquery(window).height()) - parseInt(height)) / 2) * 0.8;
			var left = (parseInt($jquery(window).width()) - parseInt(width)) / 2;		
			// resize there
			$jquery('#w2ui-popup').css({
		...
		stopMove: function (evnt) {
			...
			$jquery('#w2ui-popup').css({
				...
				'left': (parseInt($jquery('#w2ui-popup').css('left')) + parseInt(this.tmp_div_x)) + 'px',
				'top':	(parseInt($jquery('#w2ui-popup').css('top'))  + parseInt(this.tmp_div_y)) + 'px'
			});
			$jquery('#w2ui-panel').css({
				'-webkit-transition': 'none',
				'-webkit-transform': 'translate3d(0px, 0px, 0px)',
				...
				'-o-transition': 'none',
				'-o-transform': 'translate(0px, 0px)',
				'left': (parseInt($jquery('#w2ui-panel').css('left')) + parseInt(this.tmp_div_x)) + 'px',
				'top':	(parseInt($jquery('#w2ui-panel').css('top'))  + parseInt(this.tmp_div_y)) + 'px'
			});
	...
	window.w2alert = function (msg, title) {
		...
		$jquery().w2popup({
			...
			buttons : '<input type="button" value="Ok" style="width: 60px" onclick="$jquery().w2popup(\'close\');">'
		});
		$jquery('#w2ui-screenPopup #btnYes').on('click', function () {
			$jquery().w2popup('close');
			if (typeof callBack == 'function') callBack();
		});
	};
	...
	window.w2confirm = function (msg, title, callBack) {
		...
		$jquery('#w2ui-screenPopup #buttonNo').on('click', function () {
			$jquery().w2popup('close');
		});
		$jquery('#w2ui-screenPopup #buttonYes').on('click', function () {
			$jquery().w2popup('close');
			if (typeof callBack == 'function') callBack();
		});
	};
...
(function () {
	...
	$jquery.fn.w2tabs = function(method) {
		if (typeof method === 'object' || !method ) {
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				$jquery.error('The parameter "name" is required but not supplied in $jquery().w2tabs().');
				return;
			}
			if (typeof w2ui[method.name] != 'undefined') {
				$jquery.error('The parameter "name" is not unique. There are other objects already created with the same name (obj: '+ method.name +').');
			...
			$jquery.extend(object, { tabs: [], handlers: [] });
			for (var i in tabs) { object.tabs[i] = $jquery.extend({}, w2tabs.prototype.tab, tabs[i]); }		
			if ($jquery(this).length != 0) {
				object.box = $jquery(this)[0];
				$jquery(this).data('w2name', object.name);
				object.render();
			}
			// register new object
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2tabs' );
		}    
	};
	...
	w2tabs.prototype = {
		...
		insert: function (id, tab) {
			if (!$jquery.isArray(tab)) tab = [tab];
			// assume it is array
			for (var r in tab) {
				// checks
				if (String(tab[r].id) == 'undefined') {
					$jquery.error('The parameter "id" is required but not supplied. (obj: '+ this.name +')');
					return;
				}
				var unique = true;
				for (var i in this.tabs) { if (this.tabs[i].id == tab[r].id) { unique = false; break; } }
				if (!unique) {
					$jquery.error('The parameter "id='+ tab[r].id +'" is not unique within the current tabs. (obj: '+ this.name +')');
					return;
				}
				if (!w2utils.isAlphaNumeric(tab[r].id)) {
					$jquery.error('The parameter "id='+ tab[r].id +'" must be alpha-numeric + "-_". (obj: '+ this.name +')');
					return;
				}
				// add tab
				var tab = $jquery.extend({}, tab, tab[r]);
		...
		remove: function (id) {
			...
			for (var a in arguments) {
				...
				// remove from screen
				$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ tab.id).remove();
			}
			return removed;
		},
		...
		set: function (id, tab) {
			...
			$jquery.extend(this.tabs[tab], tab);
			this.refresh(id);
			return true;	
		},
		...
		refresh: function (id) {
			...
			var jq_el   = $jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ tab.id);
			...
			if (jq_el.length == 0) {
				...
				if (this.getIndex(id) != this.tabs.length-1 && $jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ this.tabs[parseInt(this.getIndex(id))+1].id).length > 0) {
					$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ this.tabs[parseInt(this.getIndex(id))+1].id).before(html);
				} else {
					$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_right').before(html);
				}
			} else {
				// refresh
				jq_el.html(tabHTML);
		...
		destroy: function () { 
			...
			delete w2ui[this.name];
			// event after
			this.trigger($jquery.extend({ phase: 'after' }));	
		},
		...
		doClick: function (id, event) {
			...
			if (eventData.stop === true) return false;
			// default action
			$jquery(this.box).find('#tabs_'+ this.name +'_tab_'+ this.active +' .w2ui-tab').removeClass('active');
			this.active = tab.id;
			// event after
			this.trigger($jquery.extend(eventData, { phase: 'after' }));
			this.refresh(id);
		...
		doClose: function(id, event) {
			...
			// default action
			var obj = this;
			$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ tab.id).css({ 
				'-webkit-transition': '.2s', 
				'-moz-transition': '2s', 
				'-ms-transition': '.2s', 
				'-o-transition': '.2s', 
				opacity: '0' });
			setTimeout(function () {
				var width = $jquery(obj.box).find('.w2ui-tabs #tabs_'+ obj.name +'_tab_'+ tab.id).width();
				$jquery(obj.box).find('.w2ui-tabs #tabs_'+ obj.name +'_tab_'+ tab.id)
					.html('<div style="width: '+ width +'px; -webkit-transition: .2s; -moz-transition: .2s; -ms-transition: .2s; -o-transition: .2s"></div>')
				setTimeout(function () {
					$jquery(obj.box).find('.w2ui-tabs #tabs_'+ obj.name +'_tab_'+ tab.id).find(':first-child').css({ 'width': '0px' });
				}, 50);
			}, 200);
			setTimeout(function () {
				obj.remove(id);		
		...
		doInsert: function(id, tab) {		
			...
			if (!$jquery.isPlainObject(tab)) return;
			// check for unique
			var unique = true;
			for (var i in this.tabs) { if (this.tabs[i].id == tab.id) { unique = false; break; } }
			if (!unique) {
				$jquery.error('The parameter "id='+ tab.id +'" is not unique within the current tabs. (obj: '+ this.name +')');
				return;
			}
			// insert simple div
			var jq_el   = $jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ tab.id);
			...
			if (tab.disabled) { addStyle += 'opacity: 0.2; -moz-opacity: 0.2; -webkit-opacity: 0.2; -o-opacity: 0.2; filter:alpha(opacity=20);'; }
			html = '<td id="tabs_'+ this.name +'_tab_'+ tab.id +'" style="'+ addStyle +'" valign="middle">'+ tabHTML +'</td>';
			if (this.getIndex(id) != this.tabs.length && $jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ this.tabs[parseInt(this.getIndex(id))].id).length > 0) {
				$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_tab_'+ this.tabs[parseInt(this.getIndex(id))].id).before(html);
			} else {
				$jquery(this.box).find('.w2ui-tabs #tabs_'+ this.name +'_right').before(html);
			}
			// -- move
			var obj = this;
			setTimeout(function () { 
				var width = $jquery('#_tmp_simple_tab').width();
				$jquery('#_tmp_tabs').remove();
				$jquery('#tabs_'+ obj.name +'_tab_'+ tab.id + ' > div').css('width', width+'px'); 
			}, 1);
			setTimeout(function () {
				// insert for real
				obj.insert(id, tab);
	...
	$jquery.extend(w2tabs.prototype, $jquery.w2event);
})();
/************************************************************************
*   Library: Web 2.0 UI for jQuery (using prototypical inheritance)
*   - Following objects defined
* 		- w2toolbar 	- toolbar widget
*		- $jquery.w2toolbar	- jQuery wrapper
*   - Dependencies: jQuery, w2utils
* 
************************************************************************/
...
(function () {
	...
	$jquery.fn.w2toolbar = function(method) {
		if (typeof method === 'object' || !method ) {
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				$jquery.error('The parameter "name" is required but not supplied in $jquery().w2toolbar().');
				return;
			}
			if (typeof w2ui[method.name] != 'undefined') {
				$jquery.error('The parameter "name" is not unique. There are other objects already created with the same name (obj: '+ method.name +').');
			...
			$jquery.extend(object, { items: [], handlers: [] });
			
			for (var i in items) { object.items[i] = $jquery.extend({}, w2toolbar.prototype.item, items[i]); }		
			if ($jquery(this).length != 0) {
				object.box = $jquery(this)[0];
				$jquery(this).data('w2name', object.name);
				object.render();
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2toolbar' );
		}    
	};
	...
	w2toolbar.prototype = {
		...
		insert: function (id, items) {
			...
			for (var o in items) {
				...
				if (typeof items[o].type == 'undefined') {
					$jquery.error('The parameter "type" is required but not supplied in w2toolbar.add() method.');
					return;
				}
				if ($jquery.inArray(String(items[o].type), ['button', 'check', 'radio', 'drop', 'menu', 'break', 'html', 'spacer']) == -1) {
					$jquery.error('The parameter "type" should be one of the following [button, check, radio, drop, menu, break, html, spacer] '+
							'in w2toolbar.add() method.');
					return;
				}
				if (typeof items[o].id == 'undefined') {
					$jquery.error('The parameter "id" is required but not supplied in w2toolbar.add() method.');
					return;
				}
				var unique = true;
				for (var i = 0; i < this.items.length; i++) { if (this.items[i].id == items[o].id) { unique = false; return; } }
				if (!unique) {
					$jquery.error('The parameter "id" is not unique within the current toolbar.');
					return;
				}
				if (!w2utils.isAlphaNumeric(items[o].id)) {
					$jquery.error('The parameter "id" must be alpha-numeric + "-_".');
					return;
				}
				// add item
				var it = $jquery.extend({}, w2toolbar.prototype.item, items[o]);
				if (id == null || typeof id == 'undefined') {
					this.items.push(it);
				} else {
					var middle = this.getIndex(id);
		...
		remove: function (id) {
			...
			for (var a in arguments) {
				...
				removed++;
				// remove from screen
				$jquery(this.box).find('.w2ui-toolbar #'+ this.name +'_item_'+ it.id).remove();
				// remove from array
		...
		set: function (id, options) {
			...
			$jquery.extend(this.items[item], options);
			this.refresh(id);
			return true;	
		},
		...
		render: function (box) {
			...
			if ($jquery('#w2ui-global-drop').length == 0) $jquery('body').append('<div id="w2ui-global-drop" class="w2ui-reset"></div>');
			// event after
			this.trigger($jquery.extend({ phase: 'after' }));	
		},
		...
		refresh: function (id) {
			...
			var jq_el = $jquery(this.box).find('.w2ui-toolbar #'+ this.name +'_item_'+ it.id);
			...
			if (jq_el.length == 0) {
				...
				if (this.getIndex(id) == this.items.length-1) {
					$jquery(this.box).find('.w2ui-toolbar #'+ this.name +'_right').before(html);
				} else {
					$jquery(this.box).find('.w2ui-toolbar #'+ this.name +'_item_'+ this.items[parseInt(this.getIndex(id))+1].id).before(html);
				}
			} else {
				// refresh
				jq_el.html(html);
		...
		doOver: function (id) {
			var it = this.get(id);
			if (it && !it.disabled) {
				$jquery('#'+ this.name +'_item_'+ it.id + ' table').addClass('over');
		...
		doOut: function (id, timeout) {
			...
			if (it && !it.disabled) {
				$jquery('#'+ this.name +'_item_'+ it.id + ' table').removeClass('over');
				...
				if (it.type == 'drop' || it.type == 'menu') { // hide drop
					...
					it.timer = setTimeout( function () {
						...
						var btn = it; 
						$jquery('#'+ this.name +'_item_'+ btn.id + ' div.w2ui-drop-box').hide();
						if ($jquery('#w2ui-global-drop').data('tb-id') == btn.id) $jquery('#w2ui-global-drop').hide();
						btn.checked = false;
						obj.refresh(btn.id);
					}, timeout);
				}
		...
		doDown: function (id) {
			...
			var it = this.get(id);
			if (it && !it.disabled) {
				$jquery('#'+ this.name +'_item_'+ it.id + ' table').addClass('down');
				// drop items
				if (it.type == 'drop' || it.type == 'menu') {
					if (!it.checked) {
						$jquery('table.w2ui-toolbar div.w2ui-drop-box').hide();
						$jquery('#'+ this.name +'_item_'+ it.id + ' div.w2ui-drop-box').show();
						if ($jquery('#w2ui-global-drop').data('tb-id') == it.id) $jquery('#w2ui-global-drop').hide();
						$jquery('#w2ui-global-drop').css({
							left: $jquery('#'+ this.name +'_item_'+ it.id + ' div.w2ui-drop-box').offset().left + 'px',
							top: $jquery('#'+ this.name +'_item_'+ it.id + ' div.w2ui-drop-box').offset().top + 'px'
						}).html(it.html).show().data('tb-id', it.id);
						// events
						var obj = this;
						$jquery('#w2ui-global-drop').unbind('mouseover').unbind('mouseout');
						$jquery('#w2ui-global-drop').bind('mouseover', function (evt) {
							var el = w2ui[obj.name]; 
							if (el) el.doDropOver(it.id);
						});
						$jquery('#w2ui-global-drop').bind('mouseout', function (evt) {
							var el = w2ui[obj.name]; 
							if (el) el.doDropOut(it.id);
						});
					} else {
						$jquery('#'+ this.name +'_item_'+ it.id + ' div.w2ui-drop-box').hide();
						if ($jquery('#w2ui-global-drop').data('tb-id') == it.id) $jquery('#w2ui-global-drop').hide();
					}
				}
			}
		...
		doDropOut: function (id, timeout) {
			...
			if (typeof id === 'undefined') {
				for (var i = 0; i < this.items.length; i++) {
					...
					$jquery('#'+ this.name +'_item_'+ this.items[i].id +' div.w2ui-drop-box').hide();
					if ($jquery('#w2ui-global-drop').data('tb-id') == this.items[i].id) $jquery('#w2ui-global-drop').hide();
				} 
			} else {	
				it.timer = setTimeout( function () {
					var el  = w2ui[obj.name];
		...
		doClick: function (id, event, menu_index) {
			...
			if (it && !it.disabled) {
				...
				$jquery('#'+ this.name +'_item_'+ it.id + ' table').removeClass('down');
				...
				if (it.type == 'radio') {
					...
					it.checked = true;
					$jquery('#'+ this.name +'_item_'+ it.id + ' table').addClass('checked');					
				}
				if (it.type == 'check' || it.type == 'drop' || it.type == 'menu') {
					...
					if (it.checked) {
						...
						$jquery('#'+ this.name +'_item_'+ it.id + ' table').removeClass('checked');					
					}
				}
				// event after
				this.trigger($jquery.extend({ phase: 'after' }));	
	...
	$jquery.extend(w2toolbar.prototype, $jquery.w2event);
})();
/************************************************************************
*   Library: Web 2.0 UI for jQuery (using prototypical inheritance)
*   - Following objects defined
* 		- w2sidebar	  - sidebar widget
*		- $jquery.w2sidebar - jQuery wrapper
*   - Dependencies: jQuery, w2utils
*
*   NICE TO HAVE
*     - group animate open
...
(function () {
	...
	$jquery.fn.w2sidebar = function(method) {
		if (typeof method === 'object' || !method ) {
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				$jquery.error('The parameter "name" is required but not supplied in $jquery().w2sidebar().');
				return;
			}
			if (typeof w2ui[method.name] != 'undefined') {
				$jquery.error('The parameter "name" is not unique. There are other objects already created with the same name (obj: '+ method.name +').');
			...
			$jquery.extend(object, { handlers: [], nodes: [] });
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2sidebar' );
		}    
	};
	...
	w2sidebar.prototype = {
		...
		insert: function (parent, before, nodes) {
			if (arguments.length == 2) {
				...
				if (ind == null) {
					$jquery.error('Cannot insert node "'+ nodes[o].text +'" because cannot find node "'+ before +'" to insert before.'); 
					return null; 
				}
				parent 	= this.get(before).parent;
			}
			if (typeof parent == 'string') parent = this.get(parent);
			if (!$jquery.isArray(nodes)) nodes = [nodes];
			for (var o in nodes) {
				if (typeof nodes[o].id == 'undefined') { 
					$jquery.error('Cannot insert node "'+ nodes[o].text +'" because it has no id.'); 
					continue;
				}
				if (this.get(this, nodes[o].id) != null) { 
					$jquery.error('Cannot insert node with id='+ nodes[o].id +' (text: '+ nodes[o].text + ') because another node with the same id already exists.'); 
					continue;
				}
				var tmp = $jquery.extend({}, w2sidebar.prototype.node, nodes[o]);
				tmp.sidebar= this;
				tmp.parent = parent;
				var nd = tmp.nodes;
		...
		set: function (parent, id, node) { 
			...
			for (var i=0; i < parent.nodes.length; i++) {
				if (parent.nodes[i].id == id) {
					...
					$jquery.extend(parent.nodes[i], node, { nodes: [] });
					if (typeof nodes != 'undefined') {
		...
		select: function (id) {
			...
			$jquery('#sidebar_'+ this.name +' #node_'+id.replace(/\./, '\\.'))
		...
		doClick: function (id, event) {
			...
			if (!nd.group && !nd.disabled) {
				$jquery('#sidebar_'+ this.name +' .w2ui-node').each(function (index, field) {
					var nid = String(field.id).replace('node_', '');
					var nd  = obj.get(nid);
					if (nd && nd.selected) {
						nd.selected = false;
						$jquery(field).removeClass('w2ui-selected').find('.w2ui-icon').removeClass('w2ui-icon-selected');
					}
				});
				$jquery('#sidebar_'+ this.name +' #node_'+id.replace(/\./, '\\.'))
					.addClass('w2ui-selected')
		...
		doOpen: function (id, event) {
			...
			// expand
			$jquery('#sidebar_'+ this.name +' #node_'+ id.replace(/\./, '\\.') +'_sub').show();
			$jquery('#sidebar_'+ this.name +' #node_'+ id.replace(/\./, '\\.') +' .w2ui-node-dots:first-child').html('<div style="width: 16px">-</div>');
			nd.expanded = true;
			// event after
			this.trigger($jquery.extend(eventData, { phase: 'after' }));
		},
		...
		doClose: function (id, event) {
			...
			if (eventData.stop === true) return false;
			// default action
			$jquery('#sidebar_'+ this.name +' #node_'+ id.replace(/\./, '\\.') +'_sub').hide();		
			$jquery('#sidebar_'+ this.name +' #node_'+ id.replace(/\./, '\\.') +' .w2ui-node-dots:first-child').html('<div style="width: 16px">+</div>');
		...
		render: function (box) {
			...
			if (!this.box) return;
			$jquery(this.box).html('<div id="sidebar_'+ this.name +'" class="w2ui-reset w2ui-sidebar" style="'+ this.style +'"></div>');
			// event after
			this.trigger($jquery.extend(eventData, { phase: 'after' }));
			// ---
			this.refresh();
		...
		refresh: function (id) {
			...
			if (node != this) {
				...
				$jquery(tmp).before('<div id="sidebar_'+ this.name + '_tmp"></div>');
				$jquery(tmp).remove();
				$jquery(nm).remove();
				$jquery('#sidebar_'+ this.name + '_tmp').before(nodeHTML);
				$jquery('#sidebar_'+ this.name + '_tmp').remove();
			}
			// refresh sub nodes
			$jquery(nm).html('');
			for (var i=0; i < node.nodes.length; i++) {
				var nodeHTML = getNodeHTML(node.nodes[i]);
				$jquery(nm).append(nodeHTML);
			...
			function getNodeHTML(nd) {
				...
				if (nd.group) {
					html = 
						...
						'				 var sp=$jquery(this).find(\'span:nth-child(1)\'); if (sp.html() == \'Hide\') sp.html(\'Show\'); else sp.html(\'Hide\');"'+
						'		onmouseout="$jquery(this).find(\'span:nth-child(1)\').css(\'color\', \'transparent\')" '+
						'		onmouseover="$jquery(this).find(\'span:nth-child(1)\').css(\'color\', \'gray\')">'+
	...
	$jquery.extend(w2sidebar.prototype, $jquery.w2event);
})();
/************************************************************************
*   Library: Web 2.0 UI for jQuery (using prototypical inheritance)
*   - Following objects defined
* 		- w2ui.w2field 	- various field controls
*		- $jquery.w2field		- jQuery wrapper
*   - Dependencies: jQuery, w2utils
* 
************************************************************************/
...
(function ($) {
	...
	$jquery.fn.w2field = function(method) {
		...
		if (w2field[method]) {
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2field' );
		}    
	};
	...
	var w2field = {
		...
		init: function (options) { 		
			var obj = w2field;
			return $jquery(this).each(function (field, index) {
				switch (options.type.toLowerCase()) {
					case 'clear': // removes any previous field type
						$jquery(this).off('keypress').off('focus').off('blur');
						if ($jquery(this).prev().hasClass('w2ui-list')) {	// if enum
							$jquery(this).prev().remove();
							$jquery(this).removeAttr('tabindex');
						}
						break;
					case 'int':
						$jquery(this).on('keypress', function (evnt) { // keyCode & charCode differ in FireFox
						...
						$jquery(this).on('blur', function (evnt)  { // keyCode & charCode differ in FireFox
							if (!w2utils.isInt(this.value)) { this.value = ''; $jquery(this).trigger('change'); } 
						});
						break;
					...
					case 'float':
						...
						$jquery(this).on('blur', function (evnt)  { 
							if (!w2utils.isFloat(this.value)) { this.value = ''; $jquery(this).trigger('change'); } 
						});
						break;
					...
					case 'money':
						...
						$jquery(this).on('blur', function (evnt)  { 
							if (!w2utils.isMoney(this.value)) { this.value = ''; $jquery(this).trigger('change'); } 
						});
						break;
					...
					case 'hex':
						...
						$jquery(this).on('blur', function (evnt)  { 
							if (!w2utils.isHex(this.value)) { this.value = ''; $jquery(this).trigger('change'); } 
						});
						break;
					...
					case 'date':
						...
						if ($jquery(this).length == 0 || $jquery('#'+$jquery(this)[0].id).length != 1) {
							console.error('The date field must have a unique id in w2field(\'date\').');
							return;
						}
						$jquery(this) // remove transtion needed for functionality
							.css( { 'transition': 'none', '-webkit-transition': 'none', '-moz-transition': 'none', '-ms-transition': 'none', '-o-transition': 'none' })
							.data("options", options)
							.on('focus', function () {
								var top  = parseFloat($jquery(obj).offset().top) + parseFloat(obj.offsetHeight);
								var left = parseFloat($jquery(obj).offset().left);
								$jquery('#global_calendar_div').remove();
								$jquery('body').append('<div id="global_calendar_div" style="position: absolute; z-index: 1600; display: none;'+
								...
								$jquery('#global_calendar_div')
									.html($jquery().w2field('calendar_get', obj.value, options))
									.css({
										left: left + 'px',
										top: top + 'px'
									})
								...
								var mtimer = setInterval(function () { 
									// monitor if moved
									if ($jquery('#global_calendar_div').data('position') != ($jquery(obj).offset().left) + 'x' + ($jquery(obj).offset().top  + obj.offsetHeight)) {
										$jquery('#global_calendar_div').css({
											'-webkit-transition': '.2s',
											left: ($jquery(obj).offset().left) + 'px',
											top : ($jquery(obj).offset().top + obj.offsetHeight) + 'px'
										}).data('position', ($jquery(obj).offset().left) + 'x' + ($jquery(obj).offset().top + obj.offsetHeight));
									}
									// monitor if destroyed
									if ($jquery(obj).length == 0 || ($jquery(obj).offset().left == 0 && $jquery(obj).offset().top == 0)) {
										clearInterval(mtimer);
										$jquery('#global_calendar_div').remove();
										return;
									}
								}, 100);
								$jquery(obj).data('mtimer', mtimer);
							})
							.on('blur', function (event) {
								if (!w2utils.isDate($jquery(obj).val())) $jquery(obj).val('');
								clearInterval($jquery(obj).data('mtimer'));
								$jquery('#global_calendar_div').remove();
							})
							.on('keypress', function (event) {
								var obj = this;
								setTimeout(function () {
									$jquery('#global_calendar_div').html( $jquery().w2field('calendar_get', obj.value, options) );
								}, 10);
							});
						break;
					...
					case 'enum':
						...
						var obj	= this;
						$jquery(obj).css({ 'border-color': 'transparent' });

						var settings = $jquery.extend({}, defaults, options);
						if ($jquery.isArray(settings.selected)) { $jquery(this).data('selected', settings.selected); } else { $jquery(this).data('selected', []); }

						// if items is array convert to an object
						if ($jquery.isArray(settings.items) && !$jquery.isPlainObject(settings.items[0])) {
							var items = [];
						...
						this.add = function (item) {
							...
							if (!$jquery.isArray(selected)) selected = [];
							...
							$jquery(this).data('last_del', null);
							$jquery(this).trigger('change');
						}

						this.show = function () {
							// insert global div
							if ($jquery('#w2ui-global-items').length == 0) {
								$jquery('body').append('<div id="w2ui-global-items" class="w2ui-reset w2ui-items" '+
							...
							var div = $jquery('#w2ui-global-items');
							div.css({
									display : 'block',
									left 	: ($jquery(obj).offset().left) + 'px',
									top 	: ($jquery(obj).offset().top + obj.offsetHeight) + 'px'
							...
							var monitor = function () { 
								...
								// monitor if moved
								if (div.data('position') != ($jquery(obj).offset().left) + 'x' + ($jquery(obj).offset().top  + obj.offsetHeight)) {
									div.css({
										'-webkit-transition': '.2s',
										left: ($jquery(obj).offset().left) + 'px',
										top : ($jquery(obj).offset().top + obj.offsetHeight) + 'px'
									...
									setTimeout(function () {
										w2field.list_render.call(obj, $jquery(obj).data('last_search'));
									}, 200);
								}
								if (div.length > 0) $jquery(obj).data('mtimer', setTimeout(monitor, 100));
							};
							$jquery(obj).data('mtimer', setTimeout(monitor, 100));
						}						

						this.hide = function () {
							clearTimeout($jquery(obj).data('mtimer'));
							$jquery('#w2ui-global-items').remove();
						}
						...
						this.refresh = function () {
							var obj = this;
							// remove all items
							$jquery($jquery(this).data('selected-div')).remove();
							// rebuild it
							var margin = 'margin-top: ' + $jquery(this).css('margin-top') + '; ' +
										 ...
										 'width: ' + (w2utils.getSize(this, 'width') 
										 		   ...
										 		   - parseInt($jquery(this).css('margin-right'))) + 'px; ';
							var html = '<div class="w2ui-list" style="'+ margin + ';">'+
									   '<ul>';
							var selected = $jquery(this).data('selected');
							for (var s in selected) {
								html += '<li style="'+ ($jquery(this).data('last_del') == s ? 'opacity: 0.5' : '') +'">'+
							...
							$jquery(this).data('selected-div', div);
							var cntHeight = w2utils.getSize(div, 'height')
								- parseInt($jquery(div).css('margin-top')) 
								- parseInt($jquery(div).css('margin-bottom'));
							if (cntHeight < 23) cntHeight = 23;
							if (cntHeight > settings.maxHeight) cntHeight = settings.maxHeight;
							$jquery(div).height(cntHeight);
							if (div.length > 0) div[0].scrollTop = 1000;
							$jquery(this).height(cntHeight);

							$jquery(div).on('click', function (event) {
								var el = event.target;
								if (el.title == 'Remove') {
									$jquery(obj).data('selected').splice($jquery(el).attr('index'), 1);
							...
							$jquery(div).find('input')
								.on('focus', function (event) {
									$jquery(div).css({ 'outline': 'auto 5px -webkit-focus-ring-color', 'outline-offset': '-2px' });
									obj.show();
									event.stopPropagation();
								})
								.on('blur', function (event) {
						...
						// init control
						$jquery(this).data('settings', settings).attr('tabindex', -1);
						this.refresh();
						break;
				}
			});
		...
		list_render: function (search) {
			...
			$jquery(this).data('last_search', search);
			if (typeof $jquery(obj).data('last_index') == 'undefined' || $jquery(obj).data('last_index') == null) $jquery(obj).data('last_index', 0);
			...
			for (var a in items) {
				...
				// if already selected
				if ($jquery.inArray(w2utils.isInt(id) ? parseInt(id) : String(id), ids) != -1 && settings.showAll !== true) continue;
				...
				if (txt1.length <= txt2.length && txt2.substr(0, txt1.length) == txt1) {
					...
					ihtml += '\n<li style="display: block; -webkit-transition: 0.2s" index="'+ a +'" value="'+ id +'" '+
							 '  onmouseover="$jquery(this).parent().find(\'li\').removeClass(\'selected\'); $jquery(this).addClass(\'selected\'); "'+
							 '	class="'+ (i % 2 ? 'w2ui-item-even' : 'w2ui-item-odd') + (i == $jquery(obj).data('last_index') ? " selected" : "") +'">'+ 
							 txt +'</li>';
					if (i == $jquery(obj).data('last_index')) $jquery(obj).data('last_item', items[a]);
					i++;
				}
			}
			ihtml += '</ul>';
			...
			var max_height = parseInt($jquery(document).height()) - parseInt(div.offset().top) - 8;
			if (parseInt(div.height()) > max_height) {
				div.css({ 
					height 	: (max_height - 5) + 'px', 
					overflow: 'show' 
				});
				$jquery(div).find('.list_items').css({
			...
			$jquery(div)
				.off('mousedown')
				.on('mousedown', function (event) {
					var id 	 = $jquery(event.target).attr('index');
					var item = settings.items[id];
					obj.add(item);
					$jquery(obj).data('last_index', 0);
					obj.refresh();
					w2field.list_render.call(obj, '');
				}
			);
			$jquery(obj).prev().find('li > input')
				.val(search)
				.css('max-width', ($jquery(div).width() - 25) + 'px')
				...
				.on('keyup', function (event) {
					var inp = this;
					setTimeout(function () { 
						var curr = $jquery(obj).data('last_index');						
						switch (event.keyCode) {
							case 38: // up
								curr--;
								if (curr < 0) curr = 0;
								$jquery(obj).data('last_index', curr);
								event.preventDefault();
								break;
							case 40: // down
								curr++;
								if (curr > $jquery(obj).data('last_max')) curr = $jquery(obj).data('last_max');
								$jquery(obj).data('last_index', curr);
								event.preventDefault();
								break;
							case 13: // enter
								if (typeof $jquery(obj).data('last_item') == 'undefined' || $jquery(obj).data('last_item') == null || noItems === true) break;
								var selected = $jquery(obj).data('selected'); 
								obj.add($jquery(obj).data('last_item'));
								// select next
								if (curr > $jquery(obj).data('last_max') - 1) curr = $jquery(obj).data('last_max')-1;
								$jquery(obj).data('last_index', curr);
								$jquery(obj).data('last_item', null);
								// refrech
								$jquery(inp).val('');
								obj.refresh();
								event.preventDefault();
							...
							case 8: // backspace
								if (inp.value == '') {
									if (typeof $jquery(obj).data('last_del') == 'undefined' || $jquery(obj).data('last_del') == null) {
										// mark for deletion
										var selected = $jquery(obj).data('selected'); 
										if (!$jquery.isArray(selected)) selected = [];
										$jquery(obj).data('last_del', selected.length-1);
										// refrech
										obj.refresh();
									} else {
										// delete marked one
										...
										$jquery(obj).data('selected', selected);
										$jquery(obj).data('last_del', null);
										// refrech
										obj.refresh();
										$jquery(obj).trigger('change');
									}
						...
						$jquery(div).height(cntHeight);
						if (div.length > 0) div[0].scrollTop = 1000;
						$jquery(obj).height(cntHeight);
						// refresh menu
						if (!(event.keyCode == 8 && inp.value == '')) { 
							$jquery(obj).prev().find('li').css('opacity', '1');
							$jquery(obj).data('last_del', null);
						}
						if ($jquery.inArray(event.keyCode, [16,91,37,39]) == -1) { // command and shift keys and arrows
		...
		calendar_get: function (date, options) {
			...
			var html =  '<table cellpadding="0" cellspacing="0" style=""><tr>' +
						'<td>'+ $jquery().w2field('calendar_month', tmp[0], tmp[2], options) +'</td>'+
		...
		calendar_next: function(date) {
			...
			var options = $jquery($jquery('#global_calendar_div.w2ui-calendar').data('el')).data('options');
			$jquery('#global_calendar_div.w2ui-calendar').html( $jquery().w2field('calendar_get', month+'/1/'+year, options) );
		},
		...
		calendar_month: function(month, year, options) {
			...
			var html  = 
				...
				'	<div style="float: left" class="w2ui-calendar-previous" onclick="$jquery().w2field(\'calendar_previous\', \''+ month +'/'+ year +'\')"> <- </div>'+
				'	<div style="float: right" class="w2ui-calendar-next" onclick="$jquery().w2field(\'calendar_next\', \''+ month +'/'+ year +'\')"> -> </div> '+ 
						months[month-1] +', '+ year + 
				'</div>'+
			...
			for (var ci=1; ci<43; ci++) {
				...
				if (options.blocked && $jquery.inArray(dt, options.blocked) != -1) {
					blocked  = ' w2ui-blocked-date';
					noSelect = true;
				} 
				html += '<td class="'+ className + blocked +'" style="'+ col + bgcol + '" id="'+ this.name +'_date_'+ dt +'" date="'+ dt +'"';
				if (noSelect === false) {
					html += 'onclick="var el = $jquery(\'#global_calendar_div.w2ui-calendar\').data(\'el\'); '+
							'	$jquery(el).val(\''+ dt +'\').trigger(\'change\').trigger(\'blur\'); '+
...
(function () {
	...
	$jquery.fn.w2form = function(method) {
		if (typeof method === 'object' || !method ) {
			var obj = this;
			// check required parameters
			if (!method || typeof method.name == 'undefined') {
				$jquery.error('The parameter "name" is required but not supplied in $jquery().w2form().');
				return;
			}
			if (typeof w2ui[method.name] != 'undefined') {
				$jquery.error('The parameter "name" is not unique. There are other objects already created with the same name (obj: '+ method.name +').');
			...
			$jquery.extend(object, { record: {}, original: {}, fields: [], tabs: {}, handlers: [] });
			if ($jquery.isArray(tabs)) {
				$jquery.extend(true, object.tabs, { tabs: tabs});
			} else {
				$jquery.extend(true, object.tabs, tabs);
			}
			// reassign variables
			for (var p in fields)  	object.fields[p]   	= $jquery.extend(true, {}, fields[p]); 
			for (var p in record) {
				if ($jquery.isPlainObject(record[p])) {
					object.record[p] = $jquery.extend(true, {}, record[p]);
				} else {
					object.record[p] = record[p];
				}
			}
			for (var p in original) {
				if ($jquery.isPlainObject(original[p])) {
					object.original[p] = $jquery.extend(true, {}, original[p]);
				} else {
			...
			object.initTabs();
			// render if necessary
			if ($jquery(this).length != 0 && !object.form_url) {
				if (!object.form_html) object.form_html = $jquery(this).html();
				object.init(this);
				object.render($jquery(this)[0]);
			} else if (object.form_url) {
				$jquery.get(object.form_url, function (data) {
					object.form_html = data;
					if ($jquery(obj).length != 0) {
						$jquery(obj).html(object.form_html);
						object.init(obj);
						object.render($jquery(obj)[0]);
					}
				});
			}
			// register new object
			...
			$jquery.error( 'Method ' +  method + ' does not exist on jQuery.w2form' );
		}    
	}		
	...
	w2form.prototype = {
		...
		init: function (el) {
			var obj = this;
			$jquery(el).find('input, textarea, select').each(function (index, el) {
				var type  = 'text';
				var name  = (typeof $jquery(el).attr('name') != 'undefined' ? $jquery(el).attr('name') : $jquery(el).attr('id'));
				...
				var value = (type == 'checkbox' || type == 'radio' ? ($jquery(el).attr('checked') ? true : false) : $jquery(el).val());

				var field = obj.get(name);
				if (field && type != 'button') {
					// find page
					var div = $jquery(el).parents('.w2ui-page');
					if (div.length > 0) {
						for (var i = 0; i < 100; i++) {
		...
		initTabs: function () {
			// -- if tabs defined
			if (!$jquery.isEmptyObject(this.tabs) && typeof this.tabs['render'] == 'undefined') {
				var obj = this;
				this.tabs = $jquery().w2tabs($jquery.extend({}, this.tabs, { name: this.name +'_tabs' }));
		...
		set: function (field, obj) {
			for (var f in this.fields) {
				if (this.fields[f].name == field) {
					$jquery.extend(this.fields[f] , obj);
					return true;
				}
			}
			return false;
				...
				complete	: function (xhr, status) {
					...
					if (typeof eventData.data != 'undefined' && eventData.data != '') {
						...
						if (data['status'] != 'success') {
							...
							obj.record 	 = $jquery.extend({}, data.record);
							obj.original = $jquery.extend({}, data.record);
						}
					}
					// event after
					obj.trigger($jquery.extend(eventData, { phase: 'after' }));
				...
				switch (field.type) {
					case 'int':
						if (this.record[field.name] && !w2utils.isInt(this.record[field.name])) {
							...
							errors.push(error);
							if (showErrors) $jquery(field.el).w2tag(error.error, { class: 'w2ui-error' });
						} 
						break;
					case 'float':
					...
					case 'hex':
						if (this.record[field.name] && !w2utils.isHex(this.record[field.name])) {
							...
							errors.push(error);
							if (showErrors) $jquery(field.el).w2tag(error, { class: 'w2ui-error' });
						} 
						break;
					case 'email':
					...
					case 'list':
					case 'enum':
						var sel = $jquery(field.el).data('selected');
						if (!$jquery.isArray(sel)) sel = [];
						switch (sel.length) {
							case 0:
		...
		save: function (postData, url, callBack) {
			...
			$jquery.extend(params, this.postData);
			$jquery.extend(params, postData);
			params.record = $jquery.extend(true, {}, this.record);
			...
			this.save_xhr = $jquery.ajax({
				...
				complete	: function (xhr, status) {
					...
					try {
						if (typeof eventData.data != 'undefined' && eventData.data != '' && $jquery.parseJSON(eventData.data) !== false) {
							var data = 'data = '+ eventData.data; 	// $jquery.parseJSON or $jquery.getJSON did not work because it expect perfect JSON data
							var data = eval(data);					//  where everything is in double quotes
							if (data['status'] != 'success') {
								console.log('ERROR: '+ data['message']);
								...
								// reset original
								obj.original = $jquery.extend({}, obj.record);
							}
						}
					} catch (e) {
						var data = {};
		...
		refresh: function () {
			...
			if (eventData.stop === true) return false;
			// default action
			$jquery(this.box).find('.w2ui-page').hide();
			$jquery(this.box).find('.w2ui-page.page-' + this.page).show();
			// refresh tabs if needed
			if (typeof this.tabs == 'object' && typeof this.tabs.refresh == 'function') {
				$jquery('#form_'+ this.name +'_tabs').show();
				this.tabs.active = this.tabs.tabs[this.page].id;
				this.tabs.refresh();
			} else {
				$jquery('#form_'+ this.name +'_tabs').hide();
			}			
			// refresh values of all fields
			for (var f in this.fields) {
				var field = this.fields[f];
				field.el = $jquery(this.box).find('[name="'+ field.name +'"]')[0];
				...
				$jquery(field.el).off('change').on('change', function () {
					var value_new 		= this.value;
					var value_previous 	= obj.record[this.name] ? obj.record[this.name] : '';
					if ($jquery(this).data('selected')) {
						var new_arr = $jquery(this).data('selected');
						var cur_arr = obj.get(this.name).selected;
						var value_new = [];
						var value_previous = [];
						if ($jquery.isArray(new_arr)) for (var i in new_arr) value_new[i] = $jquery.extend(true, {}, new_arr[i]); // clone array
						if ($jquery.isArray(cur_arr)) for (var i in cur_arr) value_previous[i] = $jquery.extend(true, {}, cur_arr[i]); // clone array
					...
					if (eventData.stop === true) { 
						$jquery(this).val(obj.record[this.name]); // return previous value
						return false;
					}
					// default action 
					var val = this.value;
				...
				if (field.required) {
					$jquery(field.el).parent().addClass('w2ui-required');
				} else {
					$jquery(field.el).parent().removeClass('w2ui-required');
				}
			}
			// attach actions on buttons
			$jquery(this.box).find('button, input[type=button]').each(function (index, el) {
				$jquery(el).off('click').on('click', function (event) {
			...
			for (var f in this.fields) {
				...
				switch (String(field.type).toLowerCase()) {
					...
					case 'date':
						...
						$jquery(field.el).w2field('date');
						break;
					case 'int':
						field.el.value = value;
						$jquery(field.el).w2field('int');
						break;
					case 'float':
						field.el.value = value;
						$jquery(field.el).w2field('float');
						break;
					case 'money':
						field.el.value = value;
						$jquery(field.el).w2field('money');
						break;
					case 'hex':
						field.el.value = value;
						$jquery(field.el).w2field('hex');
						break;
					case 'alphanumeric':
						field.el.value = value;
						$jquery(field.el).w2field('alphaNumeric');
						break;
					case 'checkbox':
						if (this.record[field.name] == true || this.record[field.name] == 1 || this.record[field.name] == 't') {
							$jquery(field.el).attr('checked', true);
						} else {
							$jquery(field.el).removeAttr('checked');
						}
						break;
					case 'password':
						// hide passwords
					...
					case 'enum':
						...
						$jquery(field.el).w2field( $jquery.extend({}, field.options, { type: 'enum', selected: v }) );
						break;
					default:
		...
		render: function (box) {
			...
			$jquery(this.box).html(html).addClass('w2ui-reset w2ui-form');
			if ($jquery(this.box).length > 0) $jquery(this.box)[0].style.cssText += this.style;
			// init tabs
			this.initTabs();
			if (typeof this.tabs == 'object' && typeof this.tabs.render == 'function') {
				this.tabs.render($jquery('#form_'+ this.name +'_tabs')[0]);
			}
			// event after
			this.trigger($jquery.extend(eventData, { phase: 'after' }));
	...
	$jquery.extend(w2form.prototype, $jquery.w2event);
})();
